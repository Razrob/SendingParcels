...
public class RulesDisplayController : MonoBehaviour
{
    ...
    public void ChangeActiveTwoPages(int offcet) // -1 or 1
    {
        ...
        if (HardLevelManager.hardLevel == 0) return;
        if (maxTwoPagesIndex > (HardLevelManager.hardLevel - 1) / 2) maxTwoPagesIndex = (HardLevelManager.hardLevel - 1) / 2;
        ...
        if (twoPages[activeTwoPagesIndex].pages[0].pageCount <= HardLevelManager.hardLevel) twoPages[activeTwoPagesIndex].pages[0].pageGameObject.SetActive(true);
        if (twoPages[activeTwoPagesIndex].pages[1].pageCount <= HardLevelManager.hardLevel) twoPages[activeTwoPagesIndex].pages[1].pageGameObject.SetActive(true);
